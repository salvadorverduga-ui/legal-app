...
async function cargarSolicitudes() {
  ...
  await cargarInfoMensajes(solicitudesActuales);
...
// solicitud elegible, para no bloquear el render con N round-trips en serie.
// Recibe la lista en vez de leer solicitudesActuales directamente porque
// cargarMisCasos() (vista cliente) todavía no asignó esa variable en el
// momento en que necesita llamar a esta función.
async function cargarInfoMensajes(lista) {
  const elegibles = lista.filter(s => s.estado === 'ACEPTADA' || s.estado === 'COMPLETADA');
...
function generarAccionesSolicitudCliente(s) {
  ...
  return `${completarHtml}${esperaResenaHtml}${accionesHtml}${generarBotonMensajes(s)}`;
}
