...
│   │   ├── mensajes.js        ← lógica de mensajes.html: bandeja de conversaciones (inbox_view), chat v2 (ver §46)
│   │   ├── conversacion.js    ← lógica de conversacion.html: vista de una conversación puntual, chat v2 (ver §46)
...
│       ├── mensajes.html      ← bandeja de conversaciones (inbox_view), chat v2 (ver §46)
│       └── conversacion.html  ← vista de pantalla completa de una conversación puntual, chat v2, sin header/footer globales (ver §46)
├── supabase/
...
## 44. Ronda de fixes y mejoras: acceso limitado, bienvenida al verificar, El Tablón, panel admin (2026-07-27)

8 módulos, cada uno con su propio commit en `main`. Migraciones nuevas: `20260727_072_notificacion_bienvenida_verificacion.sql`, `20260727_073_mi_perfil_publico_abogado.sql`, `20260727_074_fix_exception_elegir_tablon.sql` — todas aplicadas vía Supabase MCP.

### Módulo 1 — Banner detallado de verificación
`actualizarBannerVerificacionDocumentos()` (`panel-abogado.js`) se divide ahora en cuatro casos, no tres: además de VERIFICADO/RECHAZADO/SUSPENDIDO, el caso PENDIENTE se separa según `estadoVerificacionActual.doc_carnet_url` — sin documentos todavía, el banner lista explícitamente las cuatro cosas que el acceso limitado bloquea (buscar, recibir solicitudes, El Tablón, perfil público) y muestra el botón "Subir documentos"; con documentos ya recibidos, el texto cambia a "en revisión" y el botón se oculta (reintentar no tiene sentido hasta que el admin decida). El banner ganó una estructura HTML nueva (`.panel-banner__contenido`, `.panel-banner__lista`) para poder alojar la lista sin romper el layout de banner + botón a la derecha.

### Módulo 2 — Accesos deshabilitados con tooltip, no ocultos
Antes, `aplicarAccesoLimitado()` ocultaba por completo "Ver solicitudes" y "El Tablón" del dashboard para un abogado no verificado. Ahora quedan visibles pero deshabilitados, con un tooltip "Disponible tras verificación" — nuevo par de helpers genéricos en `utils.js`: `aplicarEstadoDeshabilitado(el, deshabilitado, mensaje?)` (agrega `aria-disabled` + `data-tooltip` + clase `.elemento-deshabilitado`, **sin** usar el atributo `disabled` nativo) e `inicializarTooltipsDeshabilitados()` (listener delegado en `document`, **fase de captura**, que intercepta el click antes de que llegue al propio elemento — necesario para bloquear tanto la navegación de un `<a href>` como la acción de un `<button>` con su propio listener ya enganchado, algo que `e.preventDefault()` solo no logra si el listener del target corre antes). No se usa `disabled` nativo a propósito: un elemento disabled no dispara ningún evento `click`, así que el tooltip nunca podría mostrarse al tocar en un dispositivo sin `:hover`.

`header.js` gana la opción `abogadoNoVerificado` en `inicializarHeader()` — cuando es `true` (pasada desde `panel-abogado.js` con `abogadoActual.verificacion !== 'VERIFICADO'`), los links "El Tablón"/"En seguimiento" del header se deshabilitan con el mismo mecanismo. Solo se cablea en `panel-abogado.js`, que es donde ya se resuelve `abogadoActual` de todos modos; otras páginas donde un abogado no verificado puede llegar (`editar-perfil-abogado.html`, `referidos.html`, etc.) no fueron tocadas — ahí esos links siguen funcionando y redirigen igual por `redirigirSiAbogadoNoAprobado()` (§42) si el abogado los sigue.

### Módulo 3 — "Ver mi perfil público" bloqueado con toast
`panel-abogado.js` agrega un listener de `click` sobre `#inicioVerPerfilPublico`: si `abogadoActual.verificacion !== 'VERIFICADO'`, cancela la navegación (`target="_blank"`) y muestra `toast.info(...)` en su lugar. Alcance acotado a este enlace del dashboard, tal como se pidió — el ítem homónimo del menú de avatar (`header.js`) no se tocó.

### Módulo 4 — Botones de subida de archivos personalizados
`subir-documentos.html`: cada `input[type=file]` pasa a `hidden`, con un botón propio (`btnSeleccionar<Campo>`) que lo activa vía `.click()` y un `<span>` que muestra el nombre del archivo elegido ("Ningún archivo seleccionado" por defecto, nombre real tras seleccionar, texto del botón cambia a "Cambiar archivo"). La validación (`validarArchivo()` de `utils.js`) corre apenas se elige el archivo, no solo al enviar — si falla, se limpia la selección y se muestra el error en un `<p class="campo__error">` específico de ese campo. `registro.html` no tenía campos de archivo que migrar (ya se habían quitado en §43).

### Módulo 5 — Notificación y modal de bienvenida al ser verificado
Migración `072`: el mensaje de `fn_notificar_estado_verificacion()` para `tipo='verificacion_aprobada'` cambia a uno que celebra el evento y lista qué puede hacer el abogado ahora. `notificaciones.js`/`notificaciones-pagina.js` agregan la clase `.notificacion--bienvenida` (fondo verde) cuando `n.tipo === 'verificacion_aprobada'`, incluso si ya está leída.

`panel-abogado.js` usa la propia notificación no leída como señal de "acaba de ser aprobado" (sin agregar ninguna columna nueva): `mostrarModalBienvenidaSiCorresponde()` corre al final de `inicializar()`, y solo muestra el modal si `abogadoActual.verificacion === 'VERIFICADO'` **y** existe una notificación no leída con `tipo='verificacion_aprobada'` **y** `localStorage['bienvenida_mostrada_' + perfilActual.id]` no está seteado. El botón "Comenzar" cierra el modal, marca la notificación como leída y setea la clave de `localStorage` — el modal en sí es markup estático nuevo en `panel-abogado.html` (`#modalBienvenidaVerificacion`), reutilizando las clases genéricas `.modal-confirmar-overlay`/`.modal-confirmar`/`.modal-confirmar__lista` que ya usan los modales de bloqueo/suspensión de `utils.js`, sin agregar CSS nueva para el modal en sí.

### Módulo 6 — Perfil público propio siempre visible para el dueño
Investigación vía MCP sobre el abogado reportado (`c8aca9db-f7a9-4d3f-8120-a8324f42e650`): `verificacion='VERIFICADO'`, `toggle_disponible=true`, `suscripcion_vigente_hasta` en el futuro, `visible_publico=true`, sin bloqueos — las tres condiciones de `busqueda_abogados` ya se cumplían al momento de revisar, y una consulta directa a la vista (con y sin sesión simulada) devolvió el perfil sin problema. No hizo falta ningún `UPDATE` de datos — el escenario del enunciado (suscripción vencida) no era la causa en este caso puntual.

De todos modos, la causa raíz estructural (un abogado depende de las mismas condiciones de visibilidad que un desconocido para verse a sí mismo) sí es real y puede repetirse con cualquier otra condición (`toggle_disponible` apagado, `visible_publico` en falso, etc.). Migración `073`: vista nueva `mi_perfil_publico`, mismas columnas que `busqueda_abogados` pero filtrada solo por `id = auth.uid()` (sin ninguna de las condiciones de visibilidad pública, y sin enmascarar `campos_publicos` — el dueño siempre ve sus datos completos). `api.abogados.getPerfilPropioPublico()` la consulta; `perfil-abogado.js` la usa en vez de `getAbogado(id)` cuando `sesionActual.user.id === abogadoId` (comparación resuelta con una llamada a `api.auth.getSession()`, solo cuando `perfil?.rol === 'abogado'`).

### Módulo 7 — El Tablón: botón publicar, tooltip, fix real de "elegir abogado", visualizaciones
**Botón "Publicar un caso"**: se sacó de `.tablon-layout__lateral` (que en mobile quedaba después del listado, por ser el segundo bloque del grid de una columna) y pasa a ser el primer elemento dentro de `#vistaCliente`, fuera de cualquier grid — la vista cliente ya no usa `.tablon-layout` de dos columnas (nunca tuvo filtros, solo ese botón), así que no hacía falta layout de grid ahí.

**Tooltip del ícono de ojo**: `generarContadorVisualizaciones()` (`utils.js`) agrega `title="Visualizaciones"` al `<span>` contenedor — tooltip nativo del navegador, sin componente nuevo.

**Fix "duplicate key value violates unique constraint idx_solicitud_activa_unica_tablon"**: el enunciado original apuntaba al índice, pero `idx_solicitud_activa_unica_tablon` (`estado = ANY('PENDIENTE','ACEPTADA') AND caso_tablon_id IS NOT NULL`) ya era exactamente equivalente a excluir los 5 estados terminales — no hacía falta redefinirlo. La causa real, reproducida en una tabla temporal aislada (sin tocar datos de producción) antes de tocar la función real: desde la migración `053` (§29), una solicitud DIRECTA y una del TABLON pueden coexistir activas entre el mismo cliente-abogado (índices separados). La rama `EXCEPTION WHEN unique_violation` de `fn_crear_solicitud_desde_tablon` actualizaba **todas** las solicitudes activas del par cliente-abogado sin distinguir origen — si el cliente ya tenía ambas activas con ese abogado y elegía un caso nuevo, esa `UPDATE` volvía "tablon" a la solicitud directa (antes `caso_tablon_id IS NULL`), y esa misma operación chocaba contra la fila del Tablón que ya existía, generando un segundo `unique_violation` sin ningún `EXCEPTION` que lo atrapara. Migración `074`: agrega `AND caso_tablon_id IS NOT NULL` al `WHERE` de esa `UPDATE`, para que solo pueda tocar la solicitud del Tablón que causó el conflicto real.

**`registrar_visualizacion_caso_tablon` y el supuesto 403**: verificado vía MCP que la función existe, es `SECURITY DEFINER`, y ya tiene `GRANT EXECUTE TO authenticated, anon` desde la migración `064` — una llamada simulada con sesión de un abogado real no devolvió ningún error. No se encontró ninguna causa reproducible; no se aplicó ningún cambio para este punto.

### Módulo 8 — Cédula en el panel admin: no era un problema de la vista
`admin_verificaciones_pendientes.cedula_solicitante` ya existía y ya estaba bien resuelta (`p_abogado.cedula`) desde el commit anterior a esta ronda (`ff8d6bb`, migración `20260724_071`). El bug real estaba en `generarDatosRegistro()` (`panel-admin.js`): filtraba `.filter(([, valor]) => valor)` sobre todas las filas por igual, así que cuando `cedula_solicitante` era `NULL` la fila "Cédula" desaparecía sin dejar rastro — indistinguible de "el campo no se muestra nunca".

Investigado por qué llegaba `NULL` en la práctica: `perfiles.cedula` es `UNIQUE`, y `fn_crear_perfil_en_registro` ya tenía (de antes) un fallback documentado que reintenta el `INSERT` sin cédula ante un `unique_violation`, dejando registro en `trigger_errors`. Confirmado contra datos reales: los abogados con cédula `NULL` en producción corresponden todos a colisiones de cédulas de prueba repetidas (`1111111111`, `1751733930` reutilizada en más de un registro) — comportamiento del fallback funcionando como está diseñado, no un bug de la función. Fix aplicado: la fila "Cédula" ahora se muestra siempre (nunca se oculta como las demás), con el texto "No registrada (posible cédula duplicada — revisar antes de aprobar)" cuando es `NULL`, para que el admin sepa que hay que investigar en vez de asumir que el dato nunca se pidió.

---

## 45. Ronda de QA: RLS de provincias, provincia en verificaciones, login directo, Tablón cliente (2026-07-28)

7 ítems reportados por severidad. Migraciones nuevas: `20260728_077_rls_provincias_anon.sql`, `20260728_078_fix_provincia_verificaciones_pendientes.sql` — aplicadas vía Supabase MCP. Dos de los siete ítems (3 y 7) se investigaron a fondo y no correspondían a bugs de código — se documentan igual porque la investigación en sí es la información útil para no repetirla.

### Fix 1 — RLS de `provincias` no autorizaba a `anon`
`busqueda.js`/`api.geo.getProvincias()` estaban perfectos. El bug era de RLS (capa 2, no capa 1): la tabla `provincias` tenía `GRANT SELECT` para `anon` pero la política `lectura_provincias` solo autorizaba al rol `authenticated` — para un visitante sin sesión en `busqueda.html` (página explícitamente pública), PostgREST devolvía 0 filas sin error. Confirmado con `SET LOCAL ROLE anon; SELECT count(*) FROM provincias` → 0. Fix: `ALTER POLICY lectura_provincias ON provincias TO anon, authenticated`. `cantones` no se tocó — su único consumidor (`editar-perfil-abogado.js`) ya requiere sesión.

### Fix 2 — Provincia ausente en verificaciones pendientes (cédula: no era bug)
`admin_verificaciones_pendientes.cedula_solicitante` ya leía `p_abogado.cedula` correctamente — los `NULL` de las 2 verificaciones PENDIENTE reales son la misma colisión de cédula duplicada de prueba ya diagnosticada en §44. `provincia` sí tenía un bug real: se armaba como `COALESCE(prov.nombre, e.provincia)`, donde `prov.nombre` sale de `abogados.provincia_id` — columna que `fn_crear_fila_abogado()` **nunca** setea al registrarse (solo inserta `numero_registro`/`especialidades`; `provincia_id` se completa recién si el abogado edita su perfil, normalmente después de ser aprobado) — y `e.provincia` es exclusivo de la rama estudio. Resultado: la provincia que el abogado sí declaró en el formulario de registro (guardada como texto libre en `perfiles.provincia` por `fn_crear_perfil_en_registro`) nunca se mostraba al admin. Fix: `COALESCE(prov.nombre, p_abogado.provincia, e.provincia)`.

### Fix 3 — "Salvador Cliente" en el log de acciones: dato, no bug
`admin_log_detalle` ya resuelve `admin_nombre` desde `perfiles p_admin ON p_admin.id = al.admin_id` — nunca usó `cliente_id`. Verificado: la única cuenta con `rol='admin'` en la base tiene literalmente `nombre_completo = 'Salvador Cliente'` en `perfiles` (dato de prueba, probablemente una cuenta creada primero como cliente y luego promovida a admin sin renombrar). La vista funciona correctamente; el nombre que muestra es el nombre real guardado. No se aplicó ningún cambio — renombrar esa cuenta es una decisión de datos del usuario, no algo para decidir unilateralmente.

### Fix 4 y 6 — Login directo sin pasar por la selección de rol
Unificados en un solo mecanismo porque el botón "Iniciar sesión" del header (`header.js`, compartido por todas las páginas) es la causa raíz de ambos: apuntaba a `/` liso, que en páginas como `busqueda.html`/`perfil-abogado.html` navega correctamente a la landing, pero en `index.html` mismo es un autoenlace que no hace nada visible (Fix 6).

`header.js` ahora enlaza a `/?mostrar_login=true` en el estado anónimo (toda página). `app.js` gana `abrirLoginSiCorresponde()`, llamada al final de `configurarUI()`: si el query param está presente, muestra `#seccionAuth` con la pestaña "Ingresar" activa directamente, sin pasar por las tarjetas "Soy cliente"/"Soy abogado o estudio" — se ahorra el clic intermedio que, para solo iniciar sesión, no aporta nada. `rolActivo` queda en `null` en este camino a propósito: `manejarIngresar()` solo corre el chequeo `rolCoincideConFlujo()` (que compara el rol real de la cuenta contra la tarjeta elegida) cuando `rolActivo` no es `null` — sin una tarjeta elegida no hay flujo declarado contra el cual comparar, así que se confía directamente en el rol de la base de datos para redirigir. Sin este cambio, forzar `rolActivo = 'cliente'` por defecto habría rechazado incorrectamente el login de cualquier cuenta abogado/estudio que llegara por este camino.

`perfil-abogado.html`: "Inicie sesión para contactar a este abogado" (`#seccionSinSesion`) apunta al mismo `/?mostrar_login=true`. `perfil-abogado.js` actualiza su selector de `a[href="/"]` a `a[href^="/?mostrar_login"]` para seguir guardando `redirect_after_login` antes de navegar (cubre tanto este enlace como el del header).

### Fix 5 — El Tablón del cliente en dos columnas
`vistaCliente` vuelve a usar `.tablon-layout` de dos columnas (listado izquierda, lateral derecha en desktop), igual que la vista abogado — revierte parcialmente la simplificación de §44 módulo 7, que había sacado el bloque "Publicar un caso" del layout de grid para resolver que quedara enterrado después del listado en mobile. Esta vez se resuelve distinto: `.tablon-layout__lateral--prioridad-mobile` (`order: -1` por debajo de 1024px, `order: 0` en desktop) mantiene el panel lateral como lo primero visible en mobile *sin* renunciar al layout de dos columnas en desktop — las dos versiones del requisito (mobile primero / desktop dos columnas) conviven con la misma estructura HTML. El lateral ahora aloja dos `.tablon-panel-lateral` apilados: el de siempre ("Publicar un caso") y uno nuevo, "Reglas del Tablón" (expiración a 15 días, solo abogados verificados aplican, se puede elegir a más de un aplicante, contacto revelado de inmediato al elegir, cierre manual disponible — mismas reglas ya documentadas en §17, redactadas para el cliente). `.tablon-layout__lateral` ganó `display:flex; flex-direction:column; gap` para apilarlos; no afecta a la vista abogado (un solo panel de filtros, sin cambio visual).
...
## 46. Chat v2: asunto → conversación → mensajes (2026-08-18)

Reemplaza por completo al chat interno original (chat v1, migraciones `20260817_083` a `091`, tabla `mensajes`, componente `frontend/js/chat.js`). Rediseño completo, no un fix incremental — rama `feature/chat-v2`, migraciones `20260818_092` a `099`.

**Qué pasó con el chat v1**: las migraciones `083`-`091` y la tabla `mensajes` **no se eliminaron** — siguen existiendo en la base, sin ningún GRANT ni RLS tocado, pero ya no se referencia ni una sola vez desde el frontend (verificado explícitamente en la Parte 12: cero ocurrencias de `api.chat`, `inicializarChat`, `destruirChat` o `chat.js` en todo `frontend/`). Los datos de prueba que tenía (6 filas) se borraron con `DELETE FROM mensajes WHERE true` en la Parte 12 — la tabla queda vacía pero con su esquema intacto por si hiciera falta auditarla más adelante. `admin_conversaciones_chat`, `admin_ver_mensajes_chat` y `mensajes_con_perfil` (vistas/RPC del chat v1) tampoco se tocaron; simplemente dejaron de tener filas nuevas que mostrar.

### Principio rector
Un **asunto** (`matters`) es la relación cliente-abogado nacida de una solicitud ya `ACEPTADA` (directa o de El Tablón). Tiene exactamente una **conversación** (`conversations`, `UNIQUE matter_id`), que tiene sus **mensajes** (`messages`) y sus dos **participantes** (`conversation_participants`). A diferencia del chat v1 (atado 1:1 a una fila de `solicitudes`), el asunto es una entidad propia con su propio ciclo de vida (`active` → `closed`, con reapertura), título editable por cada parte, y persiste aunque la solicitud original cambie de estado.

**Decisión de diseño clave**: `matters.source_id` es **siempre** `solicitudes.id`, tanto para origen `'directa'` como `'tablon'` — nunca `casos_tablon.id`. Motivo: CLAUDE.md §17 permite elegir a más de un abogado en el mismo caso de El Tablón, cada elección con su propia fila en `solicitudes` — usar `caso_tablon_id` como `source_id` habría roto la relación 1 asunto = 1 par cliente-abogado en cuanto hubiera 2+ abogados elegidos en el mismo caso. `source_type` queda como metadato informativo (derivado de `solicitudes.caso_tablon_id`, validado server-side contra lo que mande el RPC, nunca confiado ciegamente).

### Modelo de datos (migración `092`)
| Tabla | Qué guarda |
|---|---|
| `matters` | `client_id`/`lawyer_id` (→ `auth.users`), `source_type`/`source_id` (→ `solicitudes`, `UNIQUE`), `title_client`/`title_lawyer` (editables independientemente por cada parte), `status` (`active`/`closed`/`archived`, solo cambia vía RPC), `reopen_requested_by`/`reopen_reason` |
| `conversations` | `matter_id` (`UNIQUE`), `last_message_at` (lo mantiene el trigger `fn_actualizar_last_message_at`) |
| `conversation_participants` | `(conversation_id, user_id)` como PK compuesta, `last_read_at`, `muted_until` |
| `messages` | `conversation_id`, `sender_id`, `body`, `edited_at`/`edited_body`, `deleted_at`, `created_at`. Permanentes: sin política de `DELETE` para ningún rol |

**RLS**: mismo patrón de helpers `SECURITY DEFINER` que el resto del proyecto para evitar la recursión de RLS documentada en §41/§43 — `fn_matter_previo`/`fn_participante_previo`/`fn_message_previo` resuelven la "fila previa" para las políticas `UPDATE` con columnas congeladas (ej. el cliente solo puede tocar `title_client`/`reopen_requested_by`/`reopen_reason`, nunca `status` directamente). `messages` INSERT exige participante y `matter.status = 'active'`; UPDATE permite al emisor **o bien** editar `edited_body`/`edited_at` (solo dentro de los primeros 5 minutos) **o bien** marcar `deleted_at` (en cualquier momento) — nunca ambos en la misma escritura.

**Edición/eliminación, quién ve qué** (vista `messages_view`): el emisor que editó ve `edited_body`; el emisor que eliminó ve el literal `'Este mensaje fue eliminado'`; el receptor, en ambos casos, sigue viendo el `body` original sin importar lo que haya hecho el emisor después de enviarlo — la vista resuelve esta regla una sola vez, el frontend no la reimplementa.

Realtime habilitado en `messages` y `conversations`. Trigger `trg_matters_updated_at` (reutiliza `fn_actualizar_updated_at`, migración 001).

### RPCs (migración `092`, todas `SECURITY DEFINER`, `REVOKE FROM PUBLIC`)
`fn_crear_matter(source_type, source_id, title)` — valida solicitud `ACEPTADA` y participante, deriva `source_type` real y lo compara contra el parámetro recibido, idempotente vía `ON CONFLICT DO NOTHING` + fallback `SELECT` (mismas llamadas concurrentes que ya resolvió `fn_verificar_rate_limit`, §16). `fn_solicitar_reapertura(matter_id, reason)` — exige `status='closed'`. `fn_responder_reapertura(matter_id, aprobar)` — exige que quien responde **no** sea quien solicitó; desde la migración `098` además notifica a quien solicitó (`reapertura_aprobada`/`reapertura_rechazada`, enum extendido en la `097` — dos pasos porque Postgres no permite usar un valor de enum recién agregado en la misma transacción, mismo motivo que `verificacion_suspendida` en §43). `fn_cerrar_matter(matter_id)` — cualquiera de las dos partes.

### Validación de URLs en mensajes (migración `093`)
`fn_validar_sin_urls_message()` (trigger `BEFORE INSERT ON messages`) — mismo regex y mismo criterio que `fn_validar_sin_urls_mensaje` del chat v1 (§ fix Codex original, `http(s)://`/`ftp://`/`mailto:`/`www.`/dominios sueltos contra TLD comunes, `\y` no `\b` en el motor ARE de Postgres). Segunda capa de defensa detrás de `REGEX_URL_MENSAJE` en `api.js`; el hint `URL_NO_PERMITIDA` se traduce al mismo mensaje amigable en ambos niveles.

### Vistas de lectura
- **`inbox_view`** (migración `092`, extendida en `094` con `last_message_preview`/`last_message_sender_id`) — bandeja del usuario autenticado: un asunto por fila, `title`/`contraparte_nombre`/`contraparte_foto` resueltos según el lado del usuario, `unread_count` (mensajes después de `last_read_at`), preview del último mensaje ya resuelto con la misma lógica de `messages_view` (vía `LATERAL JOIN`).
- **`matter_detalle_view`** (migración `095`) — detalle de un asunto puntual para la página de conversación: mismo criterio de `title`/`contraparte_*` que `inbox_view`, más `client_id`/`lawyer_id` crudos y `reopen_requested_by`/`reopen_reason`. Existe porque `perfiles` RLS (§ migración 001) no deja leer el nombre de la contraparte con un `SELECT` directo — solo una vista `SECURITY DEFINER` puede resolverlo.
- **`admin_matters_chat_v2`** (migración `099`) — listado para el panel admin, ver más abajo.

### Notificaciones (migraciones `096`, `097`/`098`)
`fn_notificar_mensaje_v2()` (`AFTER INSERT ON messages`) reutiliza el tipo `mensaje_nuevo` ya existente en el enum. A diferencia del chat v1, sí hay información suficiente para decidir si conviene notificar: no notifica si el destinatario silenció la conversación (`conversation_participants.muted_until > now()`) ni si el asunto no está `active` (defensa en profundidad — la RLS de `INSERT` ya lo garantiza). `url_destino` siempre `/pages/conversacion?id=<conversation_id>` — nunca el formato viejo `?chat=true`. El título del mensaje usa `title_client`/`title_lawyer` según a quién le toca. Reapertura: ver RPCs arriba.

### API (`frontend/js/api.js`, namespace `mensajes` — no `chat`)
`getInbox()`, `getConversacion(conversationId, cursor)` (paginación por cursor `{created_at, id}` del mensaje más antiguo cargado, no por offset — evita desincronizarse si llegan mensajes nuevos entre una página y la siguiente), `enviar`/`editarMensaje`/`eliminarMensaje`, `marcarLeido`, `escuchar(conversationId, cb)` (Realtime `INSERT` únicamente — una edición o eliminación ajena no se refleja en vivo, solo al recargar), `escucharInbox(cb)`, `crearMatter(solicitudId)` (resuelve `source_type`/título internamente, ver arriba), `solicitarReapertura`/`responderReapertura`/`cerrarMatter`, `getMatter(solicitudId)` (incluye `unread_count`, resuelto con una consulta extra a `inbox_view`), `getMatterDetalle(conversationId)`, `actualizarTitulo(matterId, nuevoTitulo, soyCliente)`. Además, `notificaciones.marcarLeidaPorUrl(url, tipo)` (genérica, no específica de chat) para la supresión silenciosa de notificaciones.

### Frontend: dos páginas nuevas
- **`frontend/pages/mensajes.html`/`.js`** — bandeja de conversaciones (Todos/No leídos/Activos, búsqueda por nombre), skeleton de carga, badge de no leídos, se refresca completo (no parchea filas) ante cualquier evento de `escucharInbox()`.
- **`frontend/pages/conversacion.html`/`.js`** — vista de pantalla completa **sin el header/footer globales de la app** (deliberado: header fijo propio con volver/avatar/título editable/badge de estado/menú de tres puntos, input fijo abajo, mensajes con scroll en el medio — como una app de mensajería, no una página de marketing). Cursor de paginación con preservación de scroll, badge flotante "N mensajes nuevos ↓", menú contextual por clic derecho o long-press (Editar solo si `< 5 min`, Eliminar con `confirmar()` de `utils.js` — nunca `window.confirm`), banner de cerrado/reapertura con sus tres estados (solicitar / esperando respuesta / responder). `cancelarEscuchaInbox` sí se conecta a `beforeunload` acá (a diferencia de `mensajes.js`, donde no se pidió) porque esta página lo pidió explícitamente.

### Integración contextual (Parte 8)
Botón "Mensajes"/"Iniciar conversación" con badge de no leídos en toda tarjeta de solicitud `ACEPTADA`/`COMPLETADA` (`solicitudes-directas.js`, `solicitudes-tablon.js` — ambas vistas), resuelto en paralelo por tarjeta vía `Promise.all(api.mensajes.getMatter(...))`. Mismo patrón en `perfil-abogado.js` si el cliente ya tiene una solicitud calificante con ese abogado. Link "Mensajes" en el header (`header.js`) junto a "El Tablón"/"En seguimiento", con badge de la suma de `unread_count`, deshabilitado con tooltip para abogados no verificados. `getItemsAtencion()`/`getItemsAtencionAbogado()`/`atencion.js` leen `inbox_view` directamente en vez de cruzar notificaciones contra solicitudes (mucho más simple que el chat v1: ya no hace falta resolver el id de solicitud desde `url_destino`).

### Acceso admin auditado (Parte 11, migración `099`)
`admin_log` gana columna `matter_id` (tercera rama del `CHECK` de "exactamente una referencia según `accion`", junto a `verificacion_id` y el `solicitud_id` del chat v1 — no se reutiliza `solicitud_id` para no mezclar semánticas entre chat v1 y v2). RPC `admin_ver_mensajes_v2(matter_id)` (`SECURITY DEFINER`, revalida `es_admin()`, registra en `admin_log` con `accion='VER_MENSAJES_V2'` y devuelve el historial en una sola operación atómica — mismo patrón que `admin_ver_mensajes_chat` del chat v1, que ya había reemplazado a una RPC de dos pasos más vieja). Sin límite de 25 ni máscara de edición/eliminación: el admin ve `body`/`edited_body`/`deleted_at` crudos, tal como están en la tabla. `admin_log_detalle` se extendió (sin romper columnas existentes) para resolver también estas entradas. Pestaña "Mensajes" en `panel-admin.html` reutiliza las burbujas `.mensaje-burbuja*` de la página de conversación, con la misma convención fija de lado (cliente = izquierda, abogado = derecha) que ya usaba el chat v1 para su vista de admin. Banner: "El acceso a conversaciones privadas queda registrado en el log de administración."
